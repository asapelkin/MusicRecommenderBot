...
def text_handler(message): # обработчик текстовых сообщений
    ...
    add_button = types.InlineKeyboardButton(text="Добавить еще @todo", callback_data="add " + textMessage)
...
def showAction(call):
    ...
    if method == "artist":
        usersArrays[call.message.chat.id] = lastfm.getSimilarArtists(config.lastFmKey, artist_name, track_name)
    elif method == "track":
        usersArrays[call.message.chat.id] = lastfm.getSimilarTracks(config.lastFmKey, artist_name, track_name)
    else:
        return
...
def sendNext(call):
    ...
    num_items = min(len(usersArrays[call.message.chat.id]), stepNumItems)
    ...
    if method == "artist":
        for i in range(0, num_items):
            print(usersArrays[call.message.chat.id][i].name)
            print(usersArrays[call.message.chat.id][i].image)
            print(usersArrays[call.message.chat.id][i].url)
            print()
            mess2send = usersArrays[call.message.chat.id][i].name + " \n" + usersArrays[call.message.chat.id][i].url
        ...
        for i in range(0, num_items):
            print(usersArrays[call.message.chat.id][i].artist)
            print(usersArrays[call.message.chat.id][i].name)
            print(usersArrays[call.message.chat.id][i].image)
            print(usersArrays[call.message.chat.id][i].url)
            print()
            mess2send = usersArrays[call.message.chat.id][i].artist + " - " + usersArrays[call.message.chat.id][i].name + " \n" + \
                        usersArrays[call.message.chat.id][i].url
            bot.send_message(call.message.chat.id, mess2send, disable_web_page_preview=True)
            ## bot.send_photo(call.message.chat.id, usersArrays[call.message.chat.id][i].image)
    else:
    ...
    usersArrays[call.message.chat.id] = usersArrays[call.message.chat.id][num_items:]

    if len(usersArrays[call.message.chat.id]) > 0:
        keyboard = types.InlineKeyboardMarkup()
